import {
    ...
    getRentPerPredictedIncome_Param
...
//3-1. 임대료 / 추정매출 계산
export async function getRentPerPredictedIncome_commercial(auto: string, admin: string) : Promise<getRentPerPredictedIncome_Param>{
    //추정매출(상권)
    const predictedIncome : getSeoulEstimateIncome_Param[]= await getSeoulEstimateIncome_commercial();
    //당월 추정 매출 합(업종 고려x)
    let totalIncome : number = 0;
    ...
    //상권별 임대료
    const seoulRent : getSeoulStorePrice_Param[] = await getSeoulStorePrice();
    //지역의 임대료 담을 변수
    ...
    //시간복잡도.. 너무 오래걸림 -> 알고리즘 리팩토링(리팩토링 대상)
    for(let tmp1 of commercialDistrict_by_region){
        for(let tmp2 of seoulRent){
            if(tmp2.TRDAR_CD_NM	 === tmp1){
                totalRent += tmp2.TOT_FLPOP_CO
        }
    }
